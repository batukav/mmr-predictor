...
if __name__  == '__main__':
    ...
    parser = argparse.ArgumentParser(description="This script will collect, process and save match data from the OpenDota API.")
    ...
    parser.add_argument('--wait_time', type = int, default=900, required=False, help= "Seconds to wait between each match collection iteration. Defaults to 15 min. To evenly distribute the available API requests over 24h, set to 4536")
    parser.add_argument('--silent', type = bool, default=False, required=False, help= "Suppress info logging and tqdm output")
    ...
    logging.basicConfig(level=log_level, format='%(asctime)s %(filename)s [%(levelname)s]: %(message)s', datefmt='%H:%M')
    ...
    while True:
        ...
        parse_game_data.parse_and_dump_match_data(match_ids=latest_matches, output_dir=args.output_dir, parsed_match_ids=parsed_match_ids, silent = args.silent) 
